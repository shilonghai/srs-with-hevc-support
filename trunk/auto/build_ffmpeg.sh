...
if [[ -f ${ff_release_dir}/lib/libfdk-aac.a ]]; then
    ...
    cd fdk-aac-0.1.3 && bash autogen.sh && ./configure --prefix=${ff_release_dir} --enable-static && make -j4 ${SRS_JOBS} && make install
...
if [[ -f ${ff_release_dir}/lib/libmp3lame.a ]]; then
    ...
    cd lame-3.99.5 && ./configure --prefix=${ff_release_dir} --enable-static && make -j4 ${SRS_JOBS} && make install
...
if [[ -f ${ff_release_dir}/lib/libspeex.a ]]; then
    ...
    cd speex-1.2rc1 && ./configure --prefix=${ff_release_dir} --enable-static && make -j4 ${SRS_JOBS} && make install
